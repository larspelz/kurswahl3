	...
	<td valign="top">
		...
		<?php
		  ...
		  for ($i=0;$i<count($fach_gk);$i++) {
			...
			// Tabelle beenden und neue �ffnen
		    $o=$fach_gk[$i]['ord'][0];
			...
			// deaktiviertes Feld erzeugen, falls als PF gew�hlt
			...
			// W�hlbarkeit und ausgew�hlte Option des Faches auswerten
			$options=array();
			$waehlbar=explode(',',$fach_gk[$i]['semwaehlbar']);

			// Falls keine Wahl vorliegt, "kein" als ausgew�hlte Option einf�gen
			// sonst als w�hlbare Option einf�gen
			if (!isset($sem))
				$options[]='<option selected value="no">kein</option>';
			else
				$options[]='<option value="no">kein</option>';

			// Optionen erzeugen, gew�hlte Option (falls vorhanden) voreinstellen
			for ($k=0;$k<count($waehlbar);$k++) {
				// HACK: Wie kann das hier verallgemeinern?
				// 1+2 Semester gew�hlt
				if ($waehlbar[$k]=='12' ) {
