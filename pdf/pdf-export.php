...
// korrigiert die String-Kodierung für die PDF-Anzeige (UTF8 -> Windows)
function encfix($str) {
	...
	for($b=0;$b<count($names);$b++){
		...
		for ($t=0;$t<8;$t++) {
			...
			switch ($t) {
				case 0:
					// Leistungskurse
					// auswerten, welches PF gewählt wurde
				...
				case 6:
				case 7:
					// vier Punkte für jede Zeile, in der ein Prüfungsfach gewählt wurde
...
// erzeugt komplette Tabellenzeilen für die Seminarkurse
function doBlockAndCellSem ($p,$num,$xofs,$yofs,$tw,$th) {
	...
	for($x=0;$x<count($sNummern);$x++){
		...
		$pdf->SetXY(10+$mgl,10);
		$pdf->Cell(0,0,'Kurswahl für die gymnasiale Oberstufe');
		...
		$pdf->SetFont('Arial','',8);
		$pdf->Cell(0,0,'Schülernummer: '.$infos['snr']);
		...
		$sp_kurse="Gewünschte Sportkurse: ";
		$m=0;
		$sk=array();
